    ...
    editContract(contractId) {
        ...
        (async () => {
            try {
                ...
                // Persist schedule into localStorage so the client-form billing tab can prefill values
                localStorage.setItem('editSchedule', JSON.stringify(schedule));
                // Indicate which tab to open: 'billing'
                localStorage.setItem('openTab', 'billing');

                // Navigate to client-form where billing tab UI exists
                window.location.hash = 'client-form';
                Utils.showNotification('Opening billing form... edit values then Save Client to update (billing schedule update flow).', 'info');
